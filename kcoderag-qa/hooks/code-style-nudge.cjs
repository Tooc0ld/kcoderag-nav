...
exports.CODE_STYLE_NUDGE = "Code-style source change: before writing, load and follow $kcoderag-code-style and its compact checklist. " +
...
function stateRelativePath(host) {
...
function containedRegularFile(root, relativePath) {
    ...
    try {
        for (const [index, part] of parts.entries()) {
            ...
            if ((last && !metadata.isFile()) || (!last && !metadata.isDirectory()))
                return undefined;
        }
...
function drift(relativePath = ".") {
    return Object.freeze({
        ok: false,
        finding: Object.freeze({ code: "capability_drift", path: relativePath }),
    });
}
function exactCompositeDigest(state) {
    if (typeof state.compositeDigest !== "string" || !DIGEST_RE.test(state.compositeDigest))
        return false;
...
function singlePathEnding(paths, suffix) {
    const matches = paths.filter((candidate) => candidate.endsWith(suffix));
    return matches.length === 1 ? matches[0] : undefined;
}
function unexpectedSkillPath(root, skillPath) {
    ...
    try {
        ...
        for (const entry of skillEntries.sort((left, right) => left.name.localeCompare(right.name))) {
            if (!expectedSkillEntries.has(entry.name) ||
        ...
        if (skillEntries.length < 2 || skillEntries.length > expectedSkillEntries.size)
            return skillPath;
        const referenceEntries = fs.readdirSync(referencesRoot, { withFileTypes: true });
        const expectedReferences = new Set(REQUIRED_REFERENCE_NAMES);
...
function evaluateCodeStyleIntegrity(options) {
    try {
        ...
        if (stateBytes.length === 0 || stateBytes.length > MAX_STATE_BYTES)
            return drift(expectedStateRelativePath);
        ...
        const capabilitySections = Array.isArray(capability.sections) && capability.sections.every(safeRelativePath)
            ? capability.sections
            : undefined;
        if (capabilitySections === undefined)
            return drift(expectedStateRelativePath);
        ...
        const automaticNudge = handlerPath !== undefined && dispatcherPath !== undefined && automaticSections.length === 1
            ? "available"
            : "unsupported";
        if (hasAutomaticAssets && automaticNudge !== "available")
            return drift(expectedStateRelativePath);
        if (automaticNudge === "unsupported" && capabilitySections.length !== 0)
            return drift(expectedStateRelativePath);
        const requiredPaths = [
            skillPath,
            ...referencePaths,
        ...
        for (const [relativePath, record] of managedRecords) {
            ...
            if (metadata.size > MAX_MANAGED_ASSET_BYTES)
                return drift(relativePath);
...
function mutationPath(record) {
    if (record.kind === "add")
        return record.path;
    if (record.kind === "delete" || !record.hasContentHunk)
        return undefined;
    return record.movePath ?? record.path;
}
function nativePatchMutationPaths(command) {
    ...
    const finishCurrent = () => {
        ...
        if (path !== undefined && !mutations.includes(path))
            mutations.push(path);
        current = undefined;
    };
    for (let index = 1; index < lines.length - 1; index += 1) {
        ...
        if (addPath !== undefined || updatePath !== undefined || deletePath !== undefined) {
            ...
            if (addPath !== undefined)
                current = { kind: "add", path: addPath, hasContentHunk: true };
            else if (updatePath !== undefined)
                current = { kind: "update", path: updatePath, hasContentHunk: false };
            else if (deletePath !== undefined)
                current = { kind: "delete", path: deletePath, hasContentHunk: false };
            continue;
        }
...
function codeStyleContribution(payload, options) {
    try {
        ...
        if (evaluateCodeStyleIntegrity(options).automaticNudge !== "available")
            return undefined;
