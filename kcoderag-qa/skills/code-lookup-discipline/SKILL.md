...
|---|---|---|
| Find where a function/class/macro is defined | `mcp__kcoderag-qa__search_code` | Symbol-level, faster than grep, understands cross-file |
| Find symbols by behavior ("get player level") | `mcp__kcoderag-qa__search_code` (semantic) | grep can't do semantic search |
| See a symbol's 360° view (signature, relations, module) | `mcp__kcoderag-qa__context` | Structured, one call vs grep+read+grep |
| Find who calls X / what X calls | `mcp__kcoderag-qa__get_call_chain` | Walks the call graph, crosses Lua↔C++ |
| Impact analysis before editing X | `mcp__kcoderag-qa__get_call_chain` (callers) | Complete caller set in one call |
| Count/aggregate (e.g. functions per module) | `mcp__kcoderag-qa__cypher` | grep can't aggregate the graph |
| Custom multi-hop graph traversal | `mcp__kcoderag-qa__cypher` | Graph query grep can't express |
...
When an MCP tool returns nothing unexpectedly, call `mcp__kcoderag-qa__list_indexes`
