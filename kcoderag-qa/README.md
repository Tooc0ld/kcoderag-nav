...
- `kcoderag-navigation` provides the QA MCP projection and graph-first guidance on five hosts.
  ...
  update notice.
- `code-style-nudge` provides a short structured pre-write advisory and the canonical
  `$code-style-correction` Skill where exact host evidence supports native delivery.
...
| Frozen host row | Navigation | Code-style nudge | Evidence verdict |
| --- | --- | --- | --- |
...
| ZCode (real-host version pending) | supported | unsupported | no code-style PASS receipt; exact refusal |

Unlisted versions do not inherit code-style guidance support. Selecting `code-style-nudge` on an unsupported host returns
...
Code-style guidance also enforces complete D-15 runtime integrity before claiming its once marker: the nearest
...
- ZCode uses project MCP and `hooks.events` with `hooks.enabled: true` in `.zcode/config.json` plus a workspace Skill. Its
  ...
  success; neither hook claims code-style pre-write delivery.
...
Codex, Claude Code, and ZCode launchers start at the session cwd and select the nearest host-specific
...
## Code-style once marker and manual D-19 reset

Code-style guidance considers only structured Write/Edit/MultiEdit/apply_patch-style calls with a reliable target,
...
1. Close every related Codex, Claude Code, Cursor, OpenCode, and ZCode session.
