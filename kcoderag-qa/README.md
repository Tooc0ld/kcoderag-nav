...
- Cursor uses an always-on Rule, shared skill, and MCP configuration. It does not claim an
  equivalent navigation `PreToolUse` Hook; its native `postToolUse` response can inject a cached
  update notice through `additional_context`.
...
All four hosts share one offline update checker. The foreground reads only bounded local state and
may detach an npm Registry refresh; it never waits for network I/O. Codex and Claude Code append a
notice to their first eligible `PreToolUse` context, Cursor returns `additional_context` from
`postToolUse`, and OpenCode displays a warning toast from `tool.execute.after`. Each session/project
cycle is deduplicated, every failure stays silent and fail-open, and no host updates automatically.
The notice only suggests the exact selected-host command, for example:
