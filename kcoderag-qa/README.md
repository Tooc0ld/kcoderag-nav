...
- `kcoderag-navigation` provides the QA MCP projection, graph-first guidance, successful-call
...
`opencode.json`/`opencode.jsonc` plus `.opencode/plugins/` and `.opencode/skills/` for OpenCode.

## Exact support matrix

Host support comes only from checked-in digest-bound receipts:
...
- Successful calls write bounded secret-free markers: Codex/Claude Code use `PostToolUse`, Cursor
  uses `afterMCPExecution`, and OpenCode uses `tool.execute.after`.
...
All four hosts share an offline foreground update checker. It reads bounded local state and may
...
silent. The notice only suggests
