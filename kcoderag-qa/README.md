...
| --- | --- | --- | --- |
| Codex `0.146.1` | supported | supported | exact `UNSUPPORTED` |
| Claude Code `2.1.241` | supported | supported | exact `PASS`, native model-visible pre-write |
| Cursor `3.17.8` | supported | supported | exact `UNSUPPORTED` |
| OpenCode `1.18.23` | supported | supported | exact `UNSUPPORTED` |
| ZCode (real-host version pending) | supported | supported | no PASS receipt; `UNSUPPORTED` |

All five hosts provide the manual code-style-nudge Skill. Native automatic pre-write is available
...
conflict or drift makes the entire request fail with zero writes; there
