...
status: complete
description: Add project-scoped ZCode host support for navigation MCP and Skill, with honest unsupported Hook/code-style boundaries and five-host lifecycle coverage
---
...
configuration and all transaction/source-conflict guarantees. At this historical checkpoint, do not
claim or install project Hooks: the then-observed ZCode contract was treated as ignoring project-level
Hook configuration. Keep the capability now named `code-style-nudge` unsupported until a
version-bound native PreToolUse receipt exists.
...
zero-write behavior for `code-style-nudge`. Do not deploy Hook, marker, or update-notice runtimes
for ZCode at this checkpoint.
...
**Action:** Prove unrelated JSON preservation, conflict refusal, `code-style-nudge` zero-write refusal, navigation
...
**Action:** Document native MCP/Skill paths, the then-observed lack of executable project Hooks,
manual explicit npm updates, unsupported `code-style-nudge`, and deferred real-host receipt. At this
historical checkpoint the then-authoritative sibling guide was synchronized; current guidance is
owned only by the repository-local `docs/MCP_QA_EXPERIENCE_GUIDE.md`.
